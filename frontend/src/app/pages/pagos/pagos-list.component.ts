...
export class PagosListComponent implements OnInit {
  ...
  saldoDisponibleSeleccionado(): number {
    ...
    if (!tramite) return 0;
    // saldoPendiente ya descuenta los pagos verificados; solo restar los no verificados
    // para saber cuánto más se puede registrar sin duplicar coberturas ya confirmadas
    const unverifiedPendientes = this.pagos
      .filter(p => p.tramiteId === tramite.id && !p.verificado && p.id !== this.editingPago?.id)
      .reduce((sum, p) => sum + (p.moneda === 'USD' ? p.monto * (p.tipoCambio || 0) : p.monto), 0);
    return Math.max(0, tramite.saldoPendiente - unverifiedPendientes);
  }
  ...
  verifyOne(id: string): void {
    ...
    this.pagoService.verificar(id).subscribe({
      ...
      error: err => {
        this.verifyingIds.delete(id);
        this.notifications.fromHttpError(err, 'Error al verificar pago');
      },
    });
  }
  ...
  verifySelected(): void {
    ...
    this.pagoService.verificarBulk([...this.selectedIds]).subscribe({
      ...
      error: err => {
        this.verifyingBulk = false;
        this.notifications.fromHttpError(err, 'Error al verificar pagos');
      },
    });
  }
