...
@Component({
  ...
  template: `
    <div style="font-family: var(--font-body);">
      ...
      @if (showModal) {
        <div class="fixed inset-0 bg-black/35 z-50 flex items-start justify-center px-4 pt-[7vh] overflow-y-auto" (click)="showModal = false">
          <form class="bg-white rounded-2xl p-6 w-full max-w-[680px] shadow-2xl my-auto" (click)="$event.stopPropagation()" (ngSubmit)="save()">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <label class="flex flex-col gap-1 text-[12px] font-medium text-[#4B5162]">
                Vehículo
                <select [(ngModel)]="form.vehiculoId" name="vehiculoId" class="px-3 py-2.5 rounded-xl border border-[#E4E7EC] text-[13px] font-normal text-[#0D1017]">
              ...
              <label class="flex flex-col gap-1 text-[12px] font-medium text-[#4B5162]">
                Trámite asociado
                <select [(ngModel)]="form.tramiteId" name="tramiteId" class="px-3 py-2.5 rounded-xl border border-[#E4E7EC] text-[13px] font-normal text-[#0D1017]">
                  <option [ngValue]="null">Sin trámite</option>
                  @for (t of tramites; track t.id) {
                    <option [value]="t.id">#{{ t.numeroConsecutivo }} · {{ t.clienteApodo || '—' }} {{ t.vehiculoMarcaModelo ? '· ' + t.vehiculoMarcaModelo : '' }}</option>
                  }
                </select>
              </label>
              <label class="flex flex-col gap-1 text-[12px] font-medium text-[#4B5162]">
...
export class GastosHormigaListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.tramiteService.getList({ pageSize: 200, orderBy: 'numero', orderDir: 'desc' }).subscribe(r => this.tramites = r.items);
    this.load();
  }
